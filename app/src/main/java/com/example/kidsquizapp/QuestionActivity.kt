       ...
       database.addListenerForSingleValueEvent(object : ValueEventListener {
           ...
           override fun onDataChange(snapshot: DataSnapshot) {
               ...
               for (i in 1..10) {
                   ...
                   val eachQuestionAnswer: HashMap<String, String> =
                       snapshot.child(randomNubmer).value as HashMap<String, String>
                   Log.d("Login question no", " ${eachQuestionNO}")
                   Log.d("Login question data", " ${eachQuestionAnswer}")
                   questionArrayList.add(
                       Question2(
                           eachQuestionAnswer.get("answer1"),
                           eachQuestionAnswer.get(
                               "answer2"
                           ),
                           eachQuestionAnswer.get("answer3"),
                           eachQuestionAnswer.get("answer4"),
                           eachQuestionAnswer.get(
                               "correctanswer"
                           ),
                           eachQuestionAnswer.get("question")
                       )
                   )
               }
               ...
               questionsAdapter.setOnClickQuestionListener(object :
                   QuestionsAdapter.SubmitOnClickListener1 {
                   override fun onclick(answer: String, totalQuestion: String) {
                       ...
                       startActivity(
                           Intent(this@QuestionActivity, ResultActivity::class.java).putExtra(
                               "score",
                               answer.toString()
                           ).putExtra("total", totalQuestion.toString())
                       )
                   }
               })
           }
...
//        }
    ...
    var timer: Long = 600000
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        super.onCreateOptionsMenu(menu)
        menuInflater.inflate(R.menu.menu, menu)
        val counter: MenuItem = menu.findItem(R.id.break_timer)
        object : CountDownTimer(timer, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                val hms: String = TimeUnit.MILLISECONDS.toHours(millisUntilFinished)
                    .toString() + ":" + (TimeUnit.MILLISECONDS.toMinutes(
                    millisUntilFinished
                ) - TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(millisUntilFinished))) + ":" + (TimeUnit.MILLISECONDS.toSeconds(
                    millisUntilFinished
            ...
            override fun onFinish() {
                ...
                questionViewPager.performClick()
    ...
    override fun onSubmitClick() {
        ...
        startActivity(
            Intent(this, ResultActivity::class.java).putExtra("score", "hello")
